...
class RichTextPageRelatedLink(Orderable, AbstractRelatedLink):
    page = ParentalKey("wagtailbase.RichTextPage", related_name="related_links")


class RichTextAttachment(Orderable, AbstractAttachment):
    page = ParentalKey("wagtailbase.RichTextPage", related_name="attachments")
...
RichTextPage.promote_panels = [
    MultiFieldPanel(BaseRichTextPage.promote_panels, "Common page configuration"),
]


class HomePage(BaseRichTextPage):
    search_name = "Home Page"
    subpage_types = ["IndexPage", "RichTextPage", "BlogIndexPage", "LandingPage"]
...
class HomePageAttachment(Orderable, AbstractAttachment):
    page = ParentalKey("wagtailbase.HomePage", related_name="attachments")
...
class BlogIndexPage(BaseIndexPage):
    ...
    def posts(self):
        ...
        return BlogPost.objects.filter(live=True, path__startswith=self.path).order_by(
            "-date"
        )
    ...
    def active_months(self):
        ...
        new_dates = set([date(d["date"].year, d["date"].month, 1) for d in dates])
    ...
    @route((r"^date" r"/(?P<year>\d{4})" r"/$"))
    @route((r"^date" r"/(?P<year>\d{4})" r"/(?P<month>(?:\w+|\d{1,2}))" r"/$"))
    @route(
        (
            r"^date"
            r"/(?P<year>\d{4})"
...
class BlogIndexPageAttachment(Orderable, AbstractAttachment):
    page = ParentalKey("wagtailbase.BlogIndexPage", related_name="attachments")
...
class BlogPostAttachment(Orderable, AbstractAttachment):
    page = ParentalKey("wagtailbase.BlogPost", related_name="attachments")
