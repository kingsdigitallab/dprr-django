...
class PersonTest(TestCase):
    def setUp(self):
        ...
        self.filiations = [
            ...
            {"text": "A. or C. f. - n.", "f": ["Aulus", "Gaius", "Caius"], "n": None},
        ...
        self.other_names = [
            ...
            {"other_names": "(= Name Surname)", "other_names_plain": "Name Surname"},
    ...
    def test__other_names_plain(self):
        for on in self.other_names:
            ...
            self.assertEqual(on["other_names_plain"], self.person.other_names_plain)
...
class PraenomenTest(TestCase):
    ...
    def test__has_alternate_name(self):
        for p in self.praenomens:
            ...
            self.assertEqual(
                p["alternate_name"] is not None, praenomen.has_alternate_name()
            )

    def test__alternate_name(self):
        for p in self.praenomens:
            praenomen = Praenomen(name=p["name"], abbrev=p["abbrev"])
            self.assertEqual(p["alternate_name"], praenomen.alternate_name)
...
class DateInformationTest(TestCase):
    ...
    def test__has_ruepke_secondary_source(self):
        ...
        self.assertFalse(self.date_info_broughton.has_ruepke_secondary_source())
        self.assertTrue(self.date_info_ruepke_upper.has_ruepke_secondary_source())
        self.assertTrue(self.date_info_ruepke_lower.has_ruepke_secondary_source())
...
class PostAssertionTest(TestCase):
    def setUp(self):
        ...
        note = PersonNote(note_type=note_type, secondary_source=note_ss, text="hello")
        note.save()
        ...
        note = PersonNote(
            note_type=note_type, secondary_source=note_ss, text=self.note_text
        )
        note.save()
    ...
    def test__get_ruepke_notes(self):
        ...
        self.assertEqual(self.note_text, self.pa_ruepke_lower.get_ruepke_notes())
