...
from promrep.models import (Office, Person, PostAssertion, PostAssertionNote,
                            Praenomen, SecondarySource, Sex, Tribe,
                            TribeAssertion)
    ...
    for field in ["nomen", "filiation", "cognomen", "other_names"]:
        ...
        if getattr(person, field) == "":
            if fvalue != "":
                ...
                updated = True
                print(
                    ("DEBUG: Updated field {} for person {}").format(field, person.id)
                )
        ...
        for row_dict in csvDict:
            ...
            if pa_note_text:
                pa_note = PostAssertionNote.objects.create(
                    secondary_source=sec_source, text=pa_note_text
                )
            ...
            row_dict.update(
                {"person_id_new": person_id, "post_assertion_id": post_assertion.id}
            )
