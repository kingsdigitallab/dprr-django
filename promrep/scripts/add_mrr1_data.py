...
from promrep.models import Group, PostAssertion, Person, \
...
def parse_post_assertion_date(ap_date_info, post_year):  # noqa
    ...
    if ap_date_info == "":
        ...
        try:
            ...
            print('FATAL ERROR saving extra date info {} {} {}'.format(
...
#    for year in years[70:71]:
    for year in years:
        ...
        for office_tag in year.findAll('office'):
            ...
            if len(office_tag.find_all('office-note')) > 0:
                for onote in office_tag.find_all('office-note'):
                    if onote.has_attr('name'):
                        a_note, created = \
                            PostAssertionNote.objects.get_or_create(
            ...
            if office_tag.has_attr('footnote') or office_tag.has_attr(
                    'x_footnote'):
            ...
            for p in office_tag.find_all('person'):
                try:
                    name_str = p['name'].replace(u"’", "'").replace(
                        u"\u2013", "-").replace(u'\xb4', "'")
                except Exception as e:
                ...
                try:
                    ...
                    if person_info is None:
                        ...
                        if 'date_certainty' in person_info:
                            ap_date_info = person_info.pop(
                                'date_certainty').strip()
                        else:
                            ap_date_info = ""
                        ...
                        if 'praenomen' in person_info:
                            try:
                                ...
                                if created:
                                    person.praenomen_uncertain = \
                                        person_info.get('praenomen_uncertain',
                                                        False)

                                    if 'patrician' in person_info:
                                        person.patrician = person_info[
                                            'patrician']
                                        person.patrican_uncertain = \
                                            person_info.get(
                                    ...
                                    print("Person existed with id: {}".format(
                                        person.id))

                            except Exception as e:
                                print("ERROR creating person: {} {}".format(
                                    name_str, e.message))
                    ...
                    # creates the PostAssertion
                        ...
                        date_obj = parse_post_assertion_date(
                            ap_date_info, year_str)
                        for key, value in date_obj.iteritems():
                            setattr(post_assertion, key, value)
                        post_assertion.save()

                        # Post Person uncertain
                        if p.has_attr('assertion-certainty') or \
                                assertion_uncertain:
                            ...
                            if fnote_id in fnote_dict:
                                ...
                                try:
                                    ap_fnote = PostAssertionNote(
                                        ...
                                        text=pnote.get_text(
                                        ).strip(), secondary_source=source)
                                    ap_fnote.save()
                                    post_assertion.notes.add(ap_fnote)
                                except:
                                    print "ERROR ADDING NOTES!!!"
                            else:
                                print("ERROR adding person footnote {}".format(
                                    fnote_id))
                            ...
                            if p.findNextSibling().name == "references":
                                ...
                                for fnote_id in footnotes:
                                    if fnote_id in fnote_dict:
                                        ...
                                        apfnote = \
                                            PostAssertionNote.objects.create(
                    ...
                    print('ERROR parsing year {} {}'.format(
                        year_str, p, e.message))
