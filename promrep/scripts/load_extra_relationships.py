...
def create_or_update_person(person_idx, row_dict):  # noqa
    ...
    p_dict = {"sex": sex, "praenomen": praenomen, "re_number": row_dict[prefix + "_re"]}
...
def get_sec_source_from_abbrev_str(ssource_str):
    ...
    slist = SecondarySource.objects.filter(abbrev_name__iexact=ssource_str.strip())
    ...
    if len(slist) == 1:
        ...
        if cted:
            print(("DEBUG>>CREATED SSOURCE: {} {}".format(sec_source, ssource_str)))
...
def read_input_file(ifname):  # noqa
    ...
    writer = csv.DictWriter(
        open(log_fname, "wb"),
        ["p1_id", "p2_id", "relationshipassertion_id"] + ICSV_COLUMNS,
        dialect="excel",
        extrasaction="ignore",
    )
    writer.writeheader()
    ...
    with open(ifname, "rU") as csvfile:
        ...
        for row_dict in csvDict:
            ...
            if len(RelationshipAssertion.objects.filter(**ra_dict)):
                ...
                rel, created = RelationshipAssertion.objects.get_or_create(**ra_dict)
                ...
                if created:
                    ...
                    primary_references_str = row_dict["primary_source_refs"].strip()
                        ...
                        if created:
                            for psource in primary_references_str.split(","):
                                primary_reference = PrimarySourceReference(
                                    content_object=ra_reference, text=psource.strip()
                                )
            ...
            row_dict.update(
                {"p1_id": p1.id, "relationshipassertion_id": rel.id, "p2_id": p2.id}
            )
